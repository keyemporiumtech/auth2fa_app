...
@Component({
	selector: 'ddc-app-ki-upload-file',
	templateUrl: './ki-upload-file.component.html',
	styleUrls: ['./ki-upload-file.component.scss'],
})
export class KiUploadFileComponent extends BaseComponent {
	...
	progressInternalLoad: number = 0;
	progressExternalLoad: number = 0;
	// icons
	...
	subProgressLoad: Subscription;
	subCompleteLoad: Subscription;
	...
	ngOnInitForChildren() {
		...
		if (!this.iconError) {
			this.iconError = template.icons.iconWarning;
		}
	}
	...
	ngOnDestroyForChildren() {
		...
		if (this.subProgressLoad) {
			this.subProgressLoad.unsubscribe();
		}
		if (this.subCompleteLoad) {
			this.subCompleteLoad.unsubscribe();
		}
	}
	getClassName(): string {
